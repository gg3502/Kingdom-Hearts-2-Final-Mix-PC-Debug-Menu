...
namespace Memory
{
    ...
    bool DetourFunction32(void* pSource, void* pDestination, size_t dwLen)
    {
        always_assert(!pSource || !pDestination || dwLen < 5, "Jump size is less than 5 bytes!");
        DWORD curProtection{};
        VirtualProtect(pSource, dwLen, PAGE_EXECUTE_READWRITE, &curProtection);

        memset(pSource, 0x90, dwLen);

        uintptr_t relativeAddress = ((uintptr_t)pDestination - (uintptr_t)pSource) - 5;

        *(BYTE*)pSource = 0xE9;
        *(uint32_t*)((uintptr_t)pSource + 1) = relativeAddress;
    ...
    void* DetourFunction64(void* pSource, void* pDestination, size_t dwLen)
    {
        constexpr DWORD MinLen = 14;
        always_assert(!pSource || !pDestination || dwLen < MinLen, "Jump size is less than 14 bytes!");
    ...
    {
        // Check if module or signature is null
        if (module == nullptr) {
            LOG(L"[ERROR] Module is nullptr!\n");
            return nullptr;
        }

        if (signature == nullptr) {
            LOG(L"[ERROR] Signature is nullptr!\n");
            return nullptr;
        }
        ...
        // Logging DOS and NT headers
        auto dosHeader = (PIMAGE_DOS_HEADER)module;
        if (dosHeader == nullptr) {
            LOG(L"[ERROR] DOS Header is nullptr!\n");
            return nullptr;
        }

        auto ntHeaders = (PIMAGE_NT_HEADERS)((std::uint8_t*)module + dosHeader->e_lfanew);
        if (ntHeaders == nullptr) {
            LOG(L"[ERROR] NT Headers is nullptr!\n");
            return nullptr;
        }
        ...
        for (auto i = 0ul; i < sizeOfImage - s; ++i) {
            ...
            if (found) {
                // Optional: Add log here if you want to log successful pattern matches
                // LOG(L"[INFO] Pattern found at offset: %lu\n", i);
                return &scanBytes[i];
            }
        }

        // Log only if the pattern is not found
        LOG(L"[INFO] Pattern not found\n");
        return nullptr;
    }
    ...
    std::string GetVersionString()
    {
        ...
        auto hResInfo = FindResourceW(hInst, (LPCWSTR)MAKEINTRESOURCE(1), (LPCWSTR)RT_VERSION);
        if (!hResInfo) return {};
    ...
    std::wstring GetVersionProductName()
    {
        ...
        GetModuleFileName(baseModule, (LPWSTR)exePath, _MAX_PATH);
        auto size = GetFileVersionInfoSize((LPCWSTR)exePath, NULL);
        std::vector<char> buffer(size);
        GetFileVersionInfo((LPCWSTR)exePath, NULL, size, buffer.data());
        VerQueryValue(buffer.data(), (LPCWSTR)L"VarFileInfo\\Translation", (LPVOID*)&lpTranslate, &cbTranslate);
        ...
        wchar_t const* p;
        UINT n_chars;
        VerQueryValue(buffer.data(), (LPCWSTR)query, (LPVOID*)&p, &n_chars);
